...
e1 <- extract(LCTR, Quad24_WA, method ='simple')
...
#Apply Function 
tabs<-lapply(seq(e1), tabFunc, e1, Quad24_WA, "QUADNAME")
...
elevation <- get_elev_raster(Quad24_WA, z = 9)  #extracts elevation data per pixel per quadrangel 

plot(elevation, xlim=c(-124.75, -116.875), ylim=c(45.5, 49))  # plots elevation data 
plot(Quad24_WA, add= TRUE) # plot WA layer to see how it overlaps

### Extract information 

e2 <- extract(elevation, Quad24_WA, method ='simple') # extracts elevation data per pixel per quadrangel 
e2
#lenge2<- length(e2)
output <- matrix(unlist(e2), ncol = lenge2, byrow = FALSE) #

e2max <- extract(elevation, Quad24_WA, method ='simple', fun = max)
e2mean <- extract(elevation, Quad24_WA, method ='simple', fun = mean)
e2min <- extract(elevation, Quad24_WA, method ='simple', fun = min)
...
tabs_elevMean<-lapply(seq(e2mean), tabFunc, e2mean, Quad24_WA, "QUADNAME")
tabs_elevMean<-do.call("rbind", tabs_elevMean )
tabs_elevMean

#Elevation Minimum per quad
tabs_elevMin<-lapply(seq(e2min), tabFunc, e2min, Quad24_WA, "QUADNAME")
...
SMAX <- extract( slo,  Quad24_WA, method ='simple', fun = max )
SAVG <- extract( slo,  Quad24_WA, method ='simple', fun = mean )
ASP <- extract(asp,  Quad24_WA,  method ='simple', fun = mean )
...
SLOPE <- cbind.data.frame(Quad24_WA$QUADNAME, SMAX, SAVG, ASP)
...
BIO_1 <- extract(Bio1, Quad24_WA, method ='simple', fun = mean)
BIO_2 <- extract(Bio2, Quad24_WA, method ='simple', fun = mean)
BIO_3 <- extract(Bio3, Quad24_WA, method ='simple', fun = mean)
BIO_4 <- extract(Bio4, Quad24_WA, method ='simple', fun = mean)
BIO_5 <- extract(Bio5, Quad24_WA, method ='simple', fun = max) # Using max generated supremum (LUB)
BIO_6 <- extract(Bio6, Quad24_WA, method ='simple', fun = min) # Using max generated infimum  (GLB)
BIO_7 <- extract(Bio7, Quad24_WA, method ='simple', fun = mean)
BIO_8 <- extract(Bio8, Quad24_WA, method ='simple', fun = mean)
BIO_9 <- extract(Bio9, Quad24_WA, method ='simple', fun = mean)
BIO_10 <- extract(Bio10, Quad24_WA, method ='simple', fun = mean)
BIO_11 <- extract(Bio11, Quad24_WA, method ='simple', fun = mean)
BIO_12 <- extract(Bio12, Quad24_WA, method ='simple', fun = mean)
BIO_13 <- extract(Bio13, Quad24_WA, method ='simple', fun = mean)
BIO_14 <- extract(Bio14, Quad24_WA, method ='simple', fun = mean)
BIO_15 <- extract(Bio15, Quad24_WA, method ='simple', fun = mean)
BIO_16 <- extract(Bio16, Quad24_WA, method ='simple', fun = mean)
BIO_17 <- extract(Bio17, Quad24_WA, method ='simple', fun = mean)
BIO_18 <- extract(Bio18, Quad24_WA, method ='simple', fun = mean)
BIO_19 <- extract(Bio19, Quad24_WA, method ='simple', fun = mean)


library(tidyr)

# Combine  climate data to add to landcover data 
CLIM <- cbind.data.frame(Quad24_WA$QUADNAME, BIO_1, BIO_2, BIO_3, BIO_4, BIO_5, 
